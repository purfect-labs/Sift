...
A verified working flow with exact curl commands, timing observations, and ffmpeg fallback for audio synthesis is in `references/working-flow.md` — consult it when you need to generate music immediately without debugging the pipeline.
...
- **`minimax-music-v26` does not support `duration_seconds`** — the quote endpoint rejects it with `"This model does not support duration_seconds"`. Use `elevenlabs-music` for duration-controlled generation instead.
- **Fish Audio is TTS-only** — voice/speech generation only. Do not use it for music generation. Use Venice music endpoints or heartmula (GPU required) for music.
- **heartmula requires NVIDIA GPU** (8GB+ VRAM). Unusable on CPU-only machines — fall back to ffmpeg drone synthesis or Venice music instead.
- **`stable-audio-25` is the cheapest music model** — $0.24 for 36s vs $0.87 for `elevenlabs-music`. Output is WAV (cleaner), not MP3. Does NOT support `force_instrumental`.
- **elevenlabs-music MP3 output has a non-zero `start_time`** (~0.025s offset). When muxing into video with `-shortest`, this can cause early audio truncation in some players. **Fix:** use `stable-audio-25` (WAV, no offset), or trim the mp3 start with `-ss 0.025` before muxing.
- **`elevenlabs-music` quote for 36s = $0.87.** `stable-audio-25` quote for 36s = $0.24. Always quote before queuing and pick the cheapest model that supports your needs.
- **`stable-audio-25` outputs WAV** (6.3MB for 36s stereo 44.1kHz). Convert to MP3 before storing: `ffmpeg -i input.wav -codec:a libmp3lame -b:a 192k output.mp3`.
